...
Spróbujmmy przedstawić poniższe drzewo w notacji z powyższej definicji.
...
Potraktujmy funkcję straty $l$ jako funkcję jednego argumentu $\hat{y}^{t-1}$
...
Ponieważ, każda obserwacja jest jednoznacznie przypożądkowana do liścia, zachodzi następująca róWność
...
Następnie z definicj drzewa $f_t(x) = w_{q_t(x)}$
...
Załóżmy teraz, żę struktura drzewa $q(x)$ jest stała, wówczas optymalne wagi
...
- Weź tą struktórę która ma najmniejszą wartość funkcji celu
...
- score łączny
- koszt złożoności wynikający z dodatkowego liścia.

Oczywiście podział warto rozważać tylko wtedy gdy $Gain$ jest dodatnie.
...
## Podsumowanie - algorytm znajdowania najlepszej strukury
...
Algorytm działą tylko dla zmiennych ciągłych. Możliwe rozwiązania

- one-hot encoding
- przypisanie każdej kategorii cyfry (tracimy część inforamcji jeżeli w zmiennej
kategorialnej nie da się określić relacji pożądku)
